...
def _cuda_http_archive_impl(repository_ctx):
    ...
    if cuda_or_cudnn_version:
        ...
        if arch_key in repository_ctx.attr.relative_url_dict.keys():
            ...
            if repository_ctx.attr.is_cudnn_dist:
                url = repository_ctx.attr.cudnn_dist_path_prefix + relative_url
            else:
                url = repository_ctx.attr.cuda_dist_path_prefix + relative_url
...
def _cuda_distributions(cuda_nccl_wheel_dict):
    ...
    cuda_distributions = get_cuda_distributions()
    cudnn_distributions = get_cudnn_distributions()
...
def workspace():
    _cuda_distributions(cuda_nccl_wheel_dict = {
